...
class ClientService:
    ...
    async def update_client(self, client_id: str, client_data: ClientCreateSchema) -> ClientResponseSchema:
        ...
        client_out = {
            ...
            "created_at": db_client_full.created_at,
            "gender": db_client_full.gender,
            "features": db_client_full.features,
            "parents": [p.__dict__ for p in db_client_full.parents],
            "diagnoses": [d.__dict__ for d in db_client_full.diagnoses],
            ...
            "photo_url": db_client_full.photo_path
        }
    ...
    async def upload_client_photo(self, client_id: str, photo: UploadFile):
        ...
        filename = f"{client_id}_{photo.filename}"
        ...
        client.photo_path = file_path
        await self.db.commit()

        return {"message": "Photo uploaded successfully", "photo_path": file_path}


    async def get_client_photo(self, client_id: str) -> str:
        result = await self.db.execute(select(Client).filter(Client.id == client_id))
        client = result.scalar_one_or_none()
